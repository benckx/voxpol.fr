package fr.voxpol.wikiscrapper
...
class WikiScrapper {
    ...
    private fun fetchFirstRoundPollTables(): List<Element> {
    ...
    private fun fetchSecondRoundPollTables(): List<Element> {
        val doc = fetchDocument()
        return doc.select("table.wikitable")
            .filter { table ->
                val firstRow = table.selectFirst("tr")
    ...
    private fun parseStudies(table: Element): List<Study> {
        val rows = table.select("tr")
